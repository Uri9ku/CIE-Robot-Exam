...
- 题目：**2145 道**，已匹配知识点 **1730 道（80.7%）**
  - 其中 **391 道**为「跨等级兜底」命中（题目考到本等级词表以外的内容），字段 `fallback:true`
  - 含图题命中率 **83.5%**（537/643）
- 仍未匹配：**415 道**（含图 106 / 无图 309）

生成方式：`docx 解析 + 受控词表（优先本等级，未命中再跨等级兜底）+ 领域关键词扩展表` 离线规则匹配
（method=`rule-keyword-v3`，同义词表已按"未命中清单"高频词扩充：Arduino/端口/单位/电路/通信/程序结构等）。

字段说明：
- `knowledgePoints`：该题命中的知识点（1~3 个，按权重排序）
- `scores`：对应权重（主关键词命中 3 分 / 上一级词 2 分 / 同义词 1 分）
- `fallback`：是否由「全量词表（跨等级）」兜底命中
- `unmatched[]`：仍未命中的题（多为图片题或时事/创意类表述），用于后续视觉模型或人工补词

后续提升方向：
1. 继续扩充关键词扩展表（成本 0，主要覆盖无图未命中题）
2. 对含图未命中题（106 道）用视觉模型识别后回填（`method` 记 `vision-v1`，可加 `confidence`）
3. 人工校对回填
